...
#import "LE_2_ViewController.h"
#import "Clock_RightViewController.h"
#import "RE_1_ViewController.h"
#import "AppDelegate.h"
#import "TransitionViewController.h"
@interface LE_1_ViewController ()

@end

@implementation LE_1_ViewController
...
-(void)viewDidLoad{
    ...
    else
    {
        ...
        arrowX    =   (self.view.bounds.size.width-381)/2-20+381;
        arrowY    = (self.view.bounds.size.height-193)/2-20+193;
        aw = 40;
        ah = 40;
        arrowDis  =   120;
        
        eX= (self.view.bounds.size.width-381)/2+381;
...
//            [image setCenter:self.view.center];
          ...
          //  image.frame=CGRectMake(0.0, 0.0, 12,12);
            //image.frame=CGRectMake(0.0, 0.0, 22,22);
            //[image setCenter:self.view.center];
            [self.view addSubview:image];
        }
    }
        ...
        if (screenWidth*scale == 768 && screenHeight*scale == 1024) {
            ...
            UIImage   *bgImage = [UIImage imageNamed:@"132ppi_10px_R.png"];
            image = [[UIImageView alloc]initWithImage:bgImage];
            image.frame=CGRectMake(truncf(eX),truncf(eY) ,10,10);
           
            [self.view addSubview:image];
            
        }
        else{//for iPad3,4,iPad Air(264 ppi----2048-by-1536)
            UIImage   *bgImage = [UIImage imageNamed:@"264ppi_20px_R.png"];
            image = [[UIImageView alloc]initWithImage:bgImage];
            image.frame=CGRectMake(truncf(eX), truncf(eY), 10,10);
         //  image.frame=CGRectMake(truncf(eX),truncf(eY), 20,20);
    ...
    //Left and right eye results dictionaries initialization
    AppDelegate *appDelegate = (AppDelegate *)[[UIApplication sharedApplication]delegate];
    NSArray *values = [[NSArray alloc]initWithObjects:@"Poor",@"Astigmatism",@"Uncreative Refractive Error",@"Require Vision Correction",nil];
    
    NSArray *keys = [[NSArray alloc]initWithObjects:@"etest",@"clockdial",@"duochrome",@"nearvision", nil];
    
    appDelegate.leftEyeResult = [[NSMutableDictionary alloc] initWithObjects:values forKeys:keys];
    
    appDelegate.rightEyeResult = [[NSMutableDictionary alloc]initWithObjects:values forKeys:keys];
...
-(void)eLogic:(NSString *)directions{
    ...
    if(ans>=curInd-1){
        if(curInd < 4){
            ...
            LE_2_ViewController *controller;
            if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPhone) {
                controller=[[LE_2_ViewController alloc]initWithNibName:@"LE_2_ViewController" bundle:nil];
                
            } else {
                controller=[[LE_2_ViewController alloc]initWithNibName:@"LE_2_ViewController_iPad" bundle:nil];
            }
       ...
       [self removeDistanceNotification];
        AppDelegate *appDelegate = (AppDelegate *)[[UIApplication sharedApplication]delegate];
        appDelegate.transAudioFile = @"clock_right_eye.wav";
        appDelegate.transNib = @"Clock_RightViewController";
        
        appDelegate.transTitle = @"CLOCK DIAL RIGHT EYE";
        appDelegate.transPara1 = @"Cover your Left Eye and look at these lines and check if all lines appear equally dark? Answer is Yes / No. \n\n Request you to keep your device at a 60cm distance while taking the test.";
        ...
        /*
        RE_1_ViewController *controller;
        if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPhone) {
            controller =[[RE_1_ViewController alloc]initWithNibName:@"RE_1_ViewController" bundle:nil];
            
        } else {
            controller =[[RE_1_ViewController alloc]initWithNibName:@"RE_1_ViewController_iPad" bundle:nil];
...
-(void)distanceMeterNotificationHandler:(NSNotification*)notification{
    ...
    if([userInfo valueForKey:@"distance"]){
       //bulb.image =  [[userInfo valueForKey:@"textcolor"]isEqualToString:@"green"]?[UIImage imageNamed:@"bulb_g.jpg"]:[UIImage imageNamed:@"bulb_s.jpg"];
        ...
        NSLog(@"Min = %f, Max = %f, Dist = %f", [slider minimumValue],
